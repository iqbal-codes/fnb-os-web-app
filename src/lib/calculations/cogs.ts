...
export function calculateOPEXPerItem(
  ...
  const opexPerItem = estimatedMonthlySales > 0 ? totalMonthlyOPEX / estimatedMonthlySales : 0;
...
export function suggestSellingPrice(
  ...
  const suggested = Math.ceil(totalCOGS / (1 - targetMarginPercent / 100) / 1000) * 1000;
