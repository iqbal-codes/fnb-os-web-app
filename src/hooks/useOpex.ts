...
export function useUpdateOpex(id: string) {
  ...
  return useMutation({
    mutationFn: async (data: Partial<OpexFormData>) => {
      const response = await apiClient.patch<{ opex: OpexItem }>(`/api/opex/${id}`, data);
      return response.data.opex;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['opex'] });
...
function calculateOpexSummary(items: OpexItem[]): OpexSummary {
  ...
  const byCategory: Record<OpexCategory, number> = {} as Record<OpexCategory, number>;
  let totalMonthly = 0;

  for (const item of activeItems) {
    const monthlyAmount = toMonthlyAmount(item.amount, item.frequency);
    totalMonthly += monthlyAmount;
    byCategory[item.category] = (byCategory[item.category] || 0) + monthlyAmount;
  }
...
export function getFrequencyLabel(frequency: string): string {
  return OPEX_FREQUENCIES.find((f) => f.value === frequency)?.label || frequency;
}
