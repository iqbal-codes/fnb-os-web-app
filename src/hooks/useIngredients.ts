...
export function useIngredients(params?: { category?: string; active?: boolean }) {
...
export function useIngredient(id: string) {
  return useQuery({
    ...
    queryFn: () => apiClient.get<IngredientResponse>(`/api/ingredients/${id}`).then((r) => r.data),
    enabled: !!id,
  });
}
...
export function useCreateIngredient() {
  ...
  return useMutation({
    mutationFn: (data: CreateIngredientRequest) =>
      apiClient.post<IngredientResponse>('/api/ingredients', data).then((r) => r.data),
    onSuccess: () => {
...
export function useUpdateIngredient(id: string) {
  ...
  return useMutation({
    mutationFn: (data: UpdateIngredientRequest) =>
      apiClient.put<IngredientResponse>(`/api/ingredients/${id}`, data).then((r) => r.data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ingredientKeys.lists() });
...
export function useDeleteIngredient() {
  ...
  return useMutation({
    mutationFn: (id: string) => apiClient.delete(`/api/ingredients/${id}`).then((r) => r.data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ingredientKeys.lists() });
    },
  });
