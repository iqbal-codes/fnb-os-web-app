...
export function ExportData() {
  ...
  const exportToCSV = (data: Transaction[]) => {
    const headers = ['Order Number', 'Date', 'Total', 'Payment Type', 'Status', 'Items'];
    ...
    const csvContent =
      ...
      [headers.join(',')]
        .concat(
          data.map((row) => {
            const items = row.items?.map((i) => `${i.quantity}x ${i.menu_name}`).join('; ') || '';
            return [
              row.order_number,
  ...
  return (
    <div className='space-y-6'>
      ...
      <div>
        ...
        <p className='text-muted-foreground text-sm'>Unduh laporan penjualan dalam format CSV</p>
      </div>
      ...
      <Card className='bg-muted/50'>
        <CardContent className='text-muted-foreground p-4 text-sm'>
          <p className='text-foreground mb-1 font-medium'>Catatan:</p>
          <ul className='list-inside list-disc space-y-1'>
            <li>Laporan berisi detail transaksi, item terjual, dan nominal.</li>
            <li>Format CSV dapat dibuka dengan Microsoft Excel atau Google Sheets.</li>
