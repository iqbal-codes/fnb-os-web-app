...
export function MenuForm({ menuId }: MenuFormProps) {
  ...
  const margin =
    sellingPrice > 0
      ? ((sellingPrice - estimatedCogs) / sellingPrice) * 100
      : 0;
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {isEditing
              ...
              : "Buat item menu baru dengan resep"}
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button
            ...
            size="sm"
            onClick={() =>
              append({ ingredient_id: "", quantity: 0, unit: "gram" })
            }
          >
        ...
        <CardContent className="space-y-4">
          {fields.length === 0 && (
            <p className="text-sm text-muted-foreground text-center py-4">
              Belum ada bahan ditambahkan. Klik &quot;Tambah Bahan&quot; untuk
              memulai.
            </p>
          )}
          ...
          {fields.map((field, index) => {
            ...
            const selectedIngredientId = watch(
              `recipe.ingredients.${index}.ingredient_id`
            );
