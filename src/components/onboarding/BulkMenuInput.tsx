...
export function BulkMenuInput({ onSave, onBack }: BulkMenuInputProps) {
  ...
  const addMenu = () => {
    setMenus((prev) => [...prev, { id: `menu-${Date.now()}`, name: '', sellingPrice: 0 }]);
  };

  const updateMenu = (id: string, field: keyof MenuItem, value: string | number) => {
  ...
  const validCount = menus.filter((m) => m.name.trim() && m.sellingPrice > 0).length;
    ...
    <div className='space-y-4'>
      ...
      <div className='space-y-3'>
        ...
        <div className='grid grid-cols-[1fr_120px_40px] gap-2 px-1'>
          <Label className='text-muted-foreground text-xs'>Nama Menu</Label>
          <Label className='text-muted-foreground text-right text-xs'>Harga Jual</Label>
          <div />
        </div>

        {/* Menu rows */}
        {menus.map((menu, idx) => (
          <div key={menu.id} className='grid grid-cols-[1fr_120px_40px] items-center gap-2'>
            <Input
            ...
            <Input
              ...
              onChange={(e) => updateMenu(menu.id, 'sellingPrice', Number(e.target.value))}
      ...
      <Card className='bg-muted/50'>
        <CardContent className='p-4'>
          <div className='flex items-center justify-between'>
            <span className='text-muted-foreground text-sm'>Menu valid</span>
      ...
      <div className='flex gap-3'>
        ...
        <Button className='flex-1' onClick={handleSave} disabled={validCount === 0}>
          Simpan & Lanjut
        </Button>
      </div>
    </div>
