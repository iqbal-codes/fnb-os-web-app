...
export function MenuRanking({ period = 'month', limit = 5 }: MenuRankingProps) {
  ...
  const bestSellers = [...topMenus].sort((a, b) => b.quantitySold - a.quantitySold);
      ...
      <CardContent>
        {!hasData ? (
          ...
          <div className='space-y-4'>
            {/* Best Sellers */}
            <div>
              <p className='text-muted-foreground mb-2 text-xs font-medium'>Terlaris</p>
                ...
                {bestSellers.slice(0, 3).map((menu, i) => (
                  <div key={menu.menuId} className='flex items-center gap-3'>
                    ...
                    <div className='min-w-0 flex-1'>
                      <p className='truncate text-sm font-medium'>{menu.menuName}</p>
                      <p className='text-muted-foreground text-xs'>{menu.quantitySold} terjual</p>
                    </div>
                    <p className='text-sm font-medium'>{formatCurrency(menu.revenue)}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Margin Performance */}
            <div>
              <p className='text-muted-foreground mb-2 text-xs font-medium'>Margin Tertinggi</p>
              <div className='space-y-2'>
